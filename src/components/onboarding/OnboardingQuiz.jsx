...
// Same opening-flow accent as AuthScreen — Sky, matching the dedicated "scaffold-opening"
    ...
    <div style={{ fontFamily: "'Inter', -apple-system, sans-serif", background: PAPER_BG, minHeight: "100dvh", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      ...
      <div className="onboard-card" style={{ ...cardStyle, width: 460, maxWidth: "100%", padding: 28 }}>
        ...
        <div style={{ display: "flex", gap: 5, marginBottom: 22 }}>
          {steps.map((s, i) => (
            <div key={s} style={{ flex: 1, height: 4, borderRadius: 2, background: i <= step ? SKY : BORDER, transition: "background-color .2s ease" }} />
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 24 }}>
          ...
          <div style={{ flex: 1 }} />
          {step < lastStep ? (
            <button onClick={() => setStep((s) => s + 1)} className="btn-primary" style={{ ...primaryBtn, background: SKY }}>Next</button>
          ) : (
            <button onClick={finish} className="btn-primary" style={{ ...primaryBtn, background: SKY }}>Get started</button>
          )}
        </div>
        {step < lastStep && (

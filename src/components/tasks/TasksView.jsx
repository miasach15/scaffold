...
import { AddRow, DatePickerButton, EmptyState, List, SectionHeader, SubHeader } from "../shared/Misc";
...
export default function TasksView({ tasks, events, onAddTask, onToggleDone, onSetCategory, onRemove, onOpenTaskDetail, onSetDate, onSetStart, onUpdateGroupDueDate, onOpenFocus, inboxItems, onTurnIntoTask, onDiscardInbox, eduItems, onSetEduDone, onUpdateEduDeadline, onAddEduSession, onGoToEducation, goalActionChips, goalMilestoneChips, onToggleGoalChip, onGoToGoals, educationCategory }) {
  ...
  // pushing its due date out and there's now more room. Lands the day after the group's
  // current last step (capped the day before the due date, floored at today) with the
  // group's own uniform title — same "rename later if you want" flow as any new task.
  const addGroupStep = (group) => {
    const category = group.remainingItems[0]?.category || "Personal";
    const todayISO = toISO(new Date());
    const existingDates = group.remainingItems.map((t) => t.date).filter(Boolean).sort();
    ...
    const cap = group.groupDueDate ? dayBefore(group.groupDueDate) : null;
    let newDate = cap && dayAfterLast > cap ? cap : dayAfterLast;
    if (newDate < todayISO) newDate = todayISO;
    onAddTask({ title: group.groupTitle, date: newDate, start: null, duration: null, category, groupId: group.groupId, groupTitle: group.groupTitle, groupDueDate: group.groupDueDate, groupDueStart: group.groupDueStart, notes: null });
  };
        ...
        <div style={{ background: "#fff", border: "1px solid #ECECEC", borderRadius: 14, padding: "16px 18px", marginBottom: 16, display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              <DatePickerButton value={date} onChange={(e) => setDate(e.target.value)} title="Optional: a task with no due date just sits in Today until you finish it" placeholder="Due date" />
              {date && (

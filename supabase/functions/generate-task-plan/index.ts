// Supabase Edge Function: breaks a plain task/project (with an optional description) into
// an ordered list of concrete work steps using Claude — the lightweight version of
// generate-assignment-plan for things that aren't school assignments or full Goals
// (e.g. a work deliverable like "marketing analysis for my internship").
//
// Deploy with:  supabase functions deploy generate-task-plan
...
serve(async (req) => {
  ...
  try {
    ...
    if (!title || !title.trim()) {
      return new Response(JSON.stringify({ error: "missing task title" }), { status: 400, headers: corsHeaders });
    }
    if (!ANTHROPIC_API_KEY) {
      return new Response(JSON.stringify({ error: "ANTHROPIC_API_KEY not set" }), { status: 500, headers: corsHeaders });
    }

    const prompt = `Someone has this task/project to get done: "${title.trim()}"${dueDate ? `, due ${dueDate}` : ""}.${details && details.trim() ? `\n\nMore details:\n"""\n${details.trim()}\n"""` : ""}

Break this into an ordered list of concrete work steps to get it fully done — from the first step to the finished result. If there's not much detail given, use your best judgement about what a task like this usually involves.
